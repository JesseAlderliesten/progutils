...
expect_message(
  expect_true(
    endsWith(x = get_file_path(dir = my_tempdir, pattern = "some_filename"),
             suffix = basename(grep(pattern = "some_filename", x = my_tempfiles,
...
expect_error(
  get_file_path(dir = my_tempdir, pattern = "SOME_FILENAME", ignore_case = FALSE),
  pattern = paste0("case-insensitive match.+", basename(my_tempfiles[1]))
)

expect_error(
  get_file_path(dir = my_tempdir, pattern = "missing_filename_abcde",
                ignore_case = TRUE),
...
expect_error(
  get_file_path(dir = my_tempdir, pattern = "missing_filename_abcde", ignore_case = FALSE),
  pattern = "No case-sensitive matches to pattern 'missing_filename_abcde' are present"
)

expect_error(
  get_file_path(dir = my_tempdir, pattern = "_filename", ignore_case = TRUE),
  pattern = "Multiple matches to pattern '_filename' are present"
)

expect_error(
  get_file_path(dir = my_tempdir, pattern = "_filename", ignore_case = FALSE),
...
expect_message(
  expect_true(endsWith(
    get_file_path(dir = my_tempdir, pattern = "test_df",
                  quietly = FALSE),
    suffix = fs::path(my_tempdir, "test_df.csv")
  )),
  pattern = "Using file", strict = TRUE, fixed = TRUE)
...
expect_message(
  expect_true(endsWith(
    get_file_path(dir = my_tempdir, pattern = "te.t_df",
                  quietly = FALSE),
    suffix = fs::path(my_tempdir, "test_df.csv")
  )),
  pattern = "Using file", strict = TRUE, fixed = TRUE)
...
expect_error(get_file_path(dir = fs::path(my_tempdir, "test_dir"),
                           pattern = "abc"),
