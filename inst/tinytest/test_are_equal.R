...
out <- c(NA, rep(FALSE, 8L), rep(c(NA, NA, FALSE, TRUE, TRUE, rep(FALSE, 6L)), 2L),
...
expect_error(are_equal(x = "a", y = c(3, 4)),
             pattern = "checkinput::all_numbers(x) is not TRUE", fixed = TRUE)
expect_error(are_equal(x = c(3, 4), y = "a"),
             pattern = "checkinput::all_numbers(y) is not TRUE", fixed = TRUE)
